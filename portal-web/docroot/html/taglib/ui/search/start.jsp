...
<aui:fieldset id="searchContainer">
	...
	<aui:select inlineField="<%= true %>" label="scope" name='<%= namespace + "groupId" %>' title="scope">
		<aui:option selected="<%= (groupId == 0) %>" value="0" ><liferay-ui:message key="everything" /></aui:option>
		<aui:option selected="<%= (groupId != 0) %>" value="<%= group.getGroupId() %>" ><liferay-ui:message key='<%= "this-" + (group.isOrganization() ? "organization" : "site") %>' /></aui:option>
	</aui:select>
