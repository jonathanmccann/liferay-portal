	...
	<aui:fieldset label="layout-template">
		...
		%>
				...
				<td align="center" width="<%= 100 / CELLS_PER_ROW %>%">
					...
					<aui:input checked="<%= layoutTemplateId.equals(layoutTemplate.getLayoutTemplateId()) %>" id='<%= "layoutTemplateId" + i %>' label="<%= LanguageUtil.get(pageContext, StringUtil.replace(layoutTemplate.getLayoutTemplateId(), CharPool.UNDERLINE, CharPool.DASH), layoutTemplate.getName()) %>" name="preferences--layoutTemplateId--" type="radio" value="<%= layoutTemplate.getLayoutTemplateId() %>" />
				</td>
