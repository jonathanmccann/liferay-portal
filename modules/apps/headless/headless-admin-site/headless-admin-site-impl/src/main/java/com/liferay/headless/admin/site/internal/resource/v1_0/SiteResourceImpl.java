...
import com.liferay.headless.admin.site.dto.v1_0.RatingTypes;
...
public class SiteResourceImpl extends BaseSiteResourceImpl {
	...
	private RatingTypes _getRatingTypes(Group group) {
		return new RatingTypes() {
			{
				setBlogs(
					() -> RatingTypes.Blogs.create(
						group.getTypeSettingsProperty(
	...
	private String _getTypeSettings(
		...
		unicodeProperties.put(
			"locales",
			StringUtil.merge(site.getAvailableLanguages(), StringPool.COMMA));
		...
		RatingTypes ratingTypes = site.getRatingTypes();

		if (ratingTypes != null) {
			if (ratingTypes.getBlogs() != null) {
				unicodeProperties.put(
					"com.liferay.blogs.model.BlogsEntry_RatingsType",
					ratingTypes.getBlogs(
					).getValue());
			}
			...
			if (ratingTypes.getDocumentsAndMedia() != null) {
				unicodeProperties.put(
					"com.liferay.document.library.kernel.model." +
						"DLFileEntry_RatingsType",
					ratingTypes.getDocumentsAndMedia(
					).getValue());
			}
		...
		return new Site() {
			{
				...
				setRatingTypes(() -> _getRatingTypes(group));
				setSharingEnabled(
					() -> Boolean.parseBoolean(
				...
				setTrashEntriesMaxAgeInMinutes(
					() -> GetterUtil.getInteger(
						group.getTypeSettingsProperty("trashEntriesMaxAge")));
				setUseDefaultLanguages(
					() -> Boolean.parseBoolean(
