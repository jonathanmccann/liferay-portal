...
boolean adminSearch = ParamUtil.getBoolean(request, "adminSearch", false);
	...
	<aui:form action="<%= portletURL %>" method="post" name="selectAssetFm">
		...
		>
			...
			<liferay-ui:search-container-results>
				<c:choose>
					<c:when test="<%= adminSearch || AssetBrowserWebConfigurationValues.SEARCH_WITH_DATABASE %>">

						<%
						int assetEntriesTotal = AssetEntryLocalServiceUtil.getEntriesCount(selectedGroupIds, new long[] {assetRendererFactory.getClassNameId()}, searchTerms.getKeywords(), searchTerms.getUserName(), searchTerms.getTitle(), searchTerms.getDescription(), adminSearch, searchTerms.isAdvancedSearch(), searchTerms.isAndOperator());

						searchContainer.setTotal(assetEntriesTotal);

						List<AssetEntry> assetEntries = AssetEntryLocalServiceUtil.getEntries(selectedGroupIds, new long[] {assetRendererFactory.getClassNameId()}, searchTerms.getKeywords(), searchTerms.getUserName(), searchTerms.getTitle(), searchTerms.getDescription(), adminSearch, searchTerms.isAdvancedSearch(), searchTerms.isAndOperator(), searchContainer.getStart(), searchContainer.getEnd(), "modifiedDate", "title", "DESC", "ASC");
